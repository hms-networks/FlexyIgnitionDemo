...
public class FlexyDemo {
    ...
    private static int getChosenScenario() {
        return ( int ) demoScenarioTag.getTagValueAsLong();
    }
    ...
    private static void resetAppTags() {
        try {
            demoStopTag.setTagValueAsInt( 0 );
            demoScenarioTag.setTagValueAsInt( 0 );
        } catch ( EWException e ) {
    ...
    public static boolean isRunning() {
        return demoStopTag.getTagValueAsLong() <= 0;
    }
    ...
    private static void runDemo() {
        while ( isRunning() ) {
            ...
            int chosenScenario = getChosenScenario() - 1;
        ...
        try {
            ...
            try {
                System.out.println( "FlexyDemo was unable to locate required tags on your system. They will now be " +
                        "created." );
                FlexyDemo.copyVarLst( FlexyDemo.class.getResourceAsStream( "/VarLstFiles/s0.csv" ), FlexyDemo.class.getResourceAsStream( "/VarLstFiles/s0.txt" ) );
                demoStopTag = new TagControl( "DEMOSTOP" );
                demoScenarioTag = new TagControl( "DEMOSCENARIO" );
            } catch ( Exception ex ) {
                ex.printStackTrace();
     ...
     * @param txtSrc Stream to Copy From for TXT
     */
    static void copyVarLst( InputStream csvSrc, InputStream txtSrc ) {
        try {
            ...
            ftpClient.storeFile( "var_lst.txt", txtSrc );
            ftpClient.logout();
            ftpClient.disconnect();
    ...
    public static void main( String[] args ) {
        ...
        // CREATE DEMO
        addDemoScenario( new FlexyDemoScenarioWastewater( "Chicago, IL Wastewater Treatment Plant West" ) );
        addDemoScenario( new FlexyDemoScenarioPackageSorting( "Manchester, NH FedEx Sorting Facility" ) );
